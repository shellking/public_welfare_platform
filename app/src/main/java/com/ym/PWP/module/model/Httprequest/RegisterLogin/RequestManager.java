...
import com.ym.PWP.module.model.Httprequest.ApiParams;
...
public class RequestManager {
    ...
    private synchronized static void initRequestQueue(){
        if (mRequestQueue==null){
            //创建一个请求队列
            mRequestQueue= Volley.newRequestQueue(ShellkingLibApplication.getInstance());
        }
    }
    ...
    public static <T> void post(final String app_url, final String tag_url, final HashMap<String,String> parameter,
                                Class<T> clazz, final HttpResponseCallBack callback){
        //发送post请求服务器
        post(app_url, tag_url, parameter, clazz, callback, Request.Priority.NORMAL);
    }
    /**
     * post 请求数据
     *
    ...
    public static <T> void post(final String app_url, final String url, final HashMap<String, String> parameter, final Class<T> clazz,
                                final HttpResponseCallBack callback, Request.Priority priority) {
        if (callback != null) {
            callback.onResponseStart(url);//回调请求开始
        }
        ...
        {// 检查当前网络是否可用
            final NetworkUtils networkUtils = new NetworkUtils(ShellkingLibApplication.getInstance());
